...
export const ThemePreview: React.FC<ThemePreviewProps> = ({ viewMode, tenantId }) => {
  ...
  useEffect(() => {
    const updatePreview = async () => {
      try {
        setError(null);
        const response = await fetch(`http://localhost:3002/preview/${tenantId}`, {
          method: 'POST',
          headers: {
        ...
        if (iframeRef.current) {
          iframeRef.current.src = `http://localhost:3002/preview/${tenantId}?t=${Date.now()}`;
        }
      } catch (err) {
  ...
  return (
    <div className="flex justify-center items-start p-4 bg-gray-100 min-h-full relative">
      ...
      <div className={viewMode === 'mobile' ? 'bg-black rounded-[44px] p-2' : ''}>
        <iframe
          ref={iframeRef}
          src={`http://localhost:3002/preview/${tenantId}`}
